...
async def get_orders_within_period(start_time, end_time): 
    collection = db["order"] 
...
async def get_orders_of_all_customers():
...
async def get_delivered_orders_within_period(start_time, end_time):
