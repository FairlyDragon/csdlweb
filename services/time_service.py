...
def get_weekday_index(time: datetime):
...
def first_day_of_previous_month(current_datetime: datetime):
...
# For Pie Chart in Dashboard Center
def last_day_of_previous_month(current_datetime: datetime):
    # first day of current month
    first_day_current_month = current_datetime.replace(day=1)
    # subtract one day to get the last day of the previous month
    last_day_previous_month = first_day_current_month - timedelta(days=1)
...
def get_monday_of_current_time(current_time: datetime):
...
def get_timeline_daily(monday: datetime):
    timeline = []
...
def get_timeline_weekly(current_time: datetime):
    timeline = []
...
def get_timeline_monthly(current_time: datetime):
    timeline = []
...
def get_start_of_month(current_time: datetime):
    return current_time.replace(day=1, hour=0, minute=0, second=0)

# Get the end time of the week
def get_end_of_week(current_datetime: datetime):
