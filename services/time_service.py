...
async def get_monday_of_current_time(current_time: datetime):
    start_of_week = current_time - timedelta(days=current_time.weekday())
    
    return await (start_of_week.replace(hour = 0, minute = 0, second = 0))
...
async def get_timeline_daily(monday: datetime):
    timeline = []
    start_time = await get_start_time_of_day(monday)
    
    for i in range(7):
        etime = await get_end_time_of_day(start_time)
...
async def get_timeline_weekly(current_time: datetime):
    timeline = []
    start_time = await get_start_of_month(current_time)
    
    for i in range(4):
        etime = await get_end_of_week(start_time)
...
async def get_timeline_monthly(current_time: datetime):
    timeline = []
    start_time = await get_start_of_year(current_time)
    
    for i in range(12):
        etime = await get_end_of_month(start_time)
...
async def get_start_of_month(current_time: datetime):
    return await current_time.replace(day = 1, hour = 0, minute = 0, second = 0)

# Get the end time of the week
async def get_end_of_week(current_datetime: datetime): 
    # Find Sunday of the current week 
    end_of_week = current_datetime + timedelta(days=(6 - current_datetime.weekday()))
    # Set 23:59:59 
    end_of_week = end_of_week.replace(hour=23, minute=59, second=59) 
    
    return await end_of_week

# Get the end time of the day
async def get_end_time_of_day(current_day:datetime):
    return await current_day.replace(hour = 23, minute = 59, second = 59)
    
# Get the start time of the day IN WEEK
async def get_start_time_of_day(current_day:datetime):
    return await current_day.replace(hour = 0, minute = 0, second = 0)
 
# Get the start time of the year
async def get_start_of_year(current_time: datetime):
    return await current_time.replace(month = 1, day = 1, hour = 0, minute = 0, second = 0)

# Get the end time of the year
async def get_end_of_month(current_datetime: datetime): 
