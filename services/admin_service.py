...
async def get_total_revenue_list_in_dashboard_center(timeline: list) -> list:
    total_revenue_list = [sum(p["amount"] for p in await get_payments_within_period(time_pair[0], time_pair[1]) if p["payment_status"] == PaymentStatus.SUCCESS) for time_pair in timeline]
