...
export default function Food() {
  ...
  useEffect(() => {
    foodService
      .getFoods()
      .then((response) => {
        // Map _id thành menuitem_id
        const mappedProducts = response.data.map((product) => ({
          ...product,
          menuitem_id: product._id || product.menuitem_id, // Sử dụng _id nếu không có menuitem_id
        }));
  ...
  const handleEditProduct = (editedProduct) => {
    const menuitem_id = editedProduct._id || editedProduct.menuitem_id;
    if (!menuitem_id) {
      console.error("Cannot edit: ID is missing");
      return;
    }

    foodService.updateFood(menuitem_id, editedProduct).then((response) => {
      setProducts(
        products.map((product) =>
          product.menuitem_id === menuitem_id
            ? { ...response.data, menuitem_id: response.data._id }
            : product
        )
      );
  ...
  const handleDeleteProduct = (menuitem_id) => {
    console.log("Deleting product with ID:", menuitem_id); // Debug log

    if (!menuitem_id) {
      console.error("Cannot delete: menuitem_id is missing");
      return;
    }

    foodService
      .deleteFood(menuitem_id)
      .then(() => {
        setProducts(
          products.filter((product) => product.menuitem_id !== menuitem_id)
        );
      })
      .catch((error) => {
  ...
  const handleApplyDiscount = async () => {
    try {
      setIsLoading(true);
      await foodService.updateDiscount(discountDialog.value);

      // Refresh lại danh sách món�n sau khi cập nhật discount
      const updatedFoods = await foodService.getFoods();
                      ...
                      {getFilteredAndSortedProducts()
                        ...
                        .map((product, index) => (
                          <Draggable
                            ...
                            draggableId={String(product.menuitem_id)}
                            index={index}
                          >
                            {(provided, snapshot) => (
                              <Box
