...
const Shipper = () => {
  console.log("Initial data:", initialShippers);
  const [shippers, setShippers] = useState(() => {
    const savedShippers = localStorage.getItem("shippers");
    if (savedShippers) {
      const parsedShippers = JSON.parse(savedShippers);
      return parsedShippers.map(shipper => ({
        ...shipper,
        email: shipper.email || shipper.username,
      }));
    }
    return initialShippers;
  });
  ...
  }, [shippers, dateSort, amountSort, searchQuery, searchType]);
  ...
  const handleAddShipper = (newShipper) => {
    const shipperWithEmail = {
      ...newShipper,
      email: newShipper.email,
    };
    setShippers(prev => [shipperWithEmail, ...prev]);
  };
