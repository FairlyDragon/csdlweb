...
export default function Delivery() {
  ...
  // Handle choosing a shipper
  const handleChooseShipper = (shipperId) => {
  ...
  const handleOpenConfirmDialog = (shipper, order) => {
    setSelectedPair({ shipper, order });
    setOpenDialog(true);
  };
  ...
  const handleAutoAssign = () => {
    while (waitingShippers.length > 0 && orderManagement.length > 0) {
      const shipper = waitingShippers[0];
      const order = orderManagement[0];
      assignDelivery(shipper, order);
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
            ...
            <List sx={{ p: 0 }}>
              {waitingShippers.map((shipper) => (
                ...
                >
                  ...
                  <Button
                    onClick={() => handleOpenConfirmDialog(shipper, null)}
                    sx={{
                      bgcolor: "#4AEDC4",
                      color: "black",
            ...
            <List sx={{ p: 0 }}>
              {orderManagement.map((order) => (
                ...
                >
                  ...
                  <Button
                    onClick={() => handleOpenConfirmDialog(null, order)}
                    sx={{
                      bgcolor: "#FEF3C7",
                      color: "black",
      ...
      <Dialog open={openDialog} onClose={handleCancel}>
        ...
        <DialogContent>
          {selectedPair.shipper && selectedPair.order ? (
            <Typography>
              Are you sure you want to assign {selectedPair.shipper.name} to order {selectedPair.order.order_id}?
            </Typography>
          ) : selectedPair.shipper ? (
            <Typography>
              Please select an order to assign to {selectedPair.shipper.name}
            </Typography>
          ) : (
            <Typography>
              Please select a shipper to assign to order {selectedPair.order?.order_id}
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
          ...
          <Button 
            ...
            disabled={!selectedPair.shipper || !selectedPair.order}
            sx={{
              bgcolor: "#36B37E",
              color: "white",
              "&:hover": { bgcolor: "#2E9567" },
