...
const Calendar = () => {
  ...
  // Load events từ localStorage khi component mount
  useEffect(() => {
  ...
  // Save events vào localStorage khi có thay đổi
  useEffect(() => {
          ...
          <Box key={monthIndex}>
            ...
            >
              ...
              {days.map((day, i) => {
                ...
                const isCurrentMonth = isSameMonth(day, new Date(selectedDate.getFullYear(), monthIndex));
                    ...
                    <Typography
                      sx={{
                        ...
                        bgcolor: isSameDay(day, new Date()) ? '#1a73e8' :
                                isSameDay(day, selectedDate) ? '#e8f0fe' :
                                hasEvents ? `${dayEvents[0].color}15` : 'transparent',
                        color: isSameDay(day, new Date()) ? 'white' :
                               !isCurrentMonth ? '#bbb' :
                               isSunday || isSaturday ? '#FF4842' : '#333',
                        '&:hover': {
                          bgcolor: isSameDay(day, new Date()) ? '#1a73e8' : 
                                  hasEvents ? `${dayEvents[0].color}30` : 'rgba(0,0,0,0.04)'
                        }
                      }}
                    >
                      {format(day, 'd')}
  ...
  // Cập nhật trong YearView
  const renderYearEvents = (day, events) => {
    const dayEvents = events.filter((event) => isSameDay(day, event.start));
    return dayEvents
      .slice(0, 3)
      .map((event) => <YearEventItem key={event.id} event={event} />);
  };
