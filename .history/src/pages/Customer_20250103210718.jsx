...
const Customer = () => {
  ...
  const handleAddCustomer = (newCustomer) => {
    const customerToAdd = {
      customer_id: Date.now().toString().slice(-5), // Tạo ID 5 số
      ...newCustomer,
  ...
  useEffect(() => {
    ...
    if (dateSort === "newest") {
      ...
      result = [...customers].sort((a, b) => {
        const dateA = new Date(a.created_at);
        const dateB = new Date(b.created_at);
        return dateA.getTime() - dateB.getTime(); // Sắp xếp tăng dần (cũ nhất lên đầu)
      });
    }

    // ... other filters ...
    setFilteredCustomers(result);
  }, [dateSort, customers]); // Chỉ theo dõi dateSort và customers
