...
const Customer = () => {
  ...
  const [customers, setCustomers] = useState(() => {
    ...
    // Nếu không có dữ liệu trong localStorage thì dùng initialCustomers
    if (!savedCustomers) {
  ...
  // 2. Khởi tạo filteredCustomers với toàn bộ dữ liệu
  const [filteredCustomers, setFilteredCustomers] = useState(customers);

  // 3. Cập nhật filteredCustomers khi customers thay đổi
  useEffect(() => {
    setFilteredCustomers(customers);
  }, [customers]);

  // 4. Hàm format date an toàn
  ...
  const handleAddCustomer = (newCustomer) => {
    const customerToAdd = {
      customer_id: String(Date.now()).slice(-5), // Tạo ID 5 số
