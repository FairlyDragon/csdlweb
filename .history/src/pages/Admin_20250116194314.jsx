...
export default function Admin() {
  ...
  const handleStatusChange = async (id) => {
    try {
      ...
      const newStatus = !currentAdmin.status;
        ...
        admins.map((admin) =>
          admin.id === id
            ? {
                ...admin,
                status: newStatus,
                permission: newStatus, // Cập nhật cả permission khi thay đổi status
              }
            : admin
        )
      );
  ...
  const handlePermissionChange = async (id) => {
    try {
      const currentAdmin = admins.find((admin) => admin.id === id);
      const newPermission = !currentAdmin.permission;

      await AdminService.updateAdminRole(id, newPermission ? "admin" : "user");
        ...
        admins.map((admin) =>
          admin.id === id
            ? {
                ...
                status: newPermission, // Cập nhật cả status khi thay đổi permission
              }
            : admin
        )
      );
