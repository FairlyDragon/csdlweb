...
import { Button, Typography, Paper, Box, Tabs, Tab, Pagination } from "@mui/material";
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 2, boxShadow: 1 }}>
      ...
      <Box sx={{ bgcolor: "white", borderRadius: 1 }}>
        ...
        {activeTab === 0 ? (
          <>
            <VoucherAvailable vouchers={paginatedVouchers(getFilteredVouchers(false))} />
            <Box sx={{ mt: 3, display: "flex", justifyContent: "center" }}>
              <Pagination
                count={Math.ceil(getFilteredVouchers(false).length / itemsPerPage)}
                page={page}
          ...
          <>
            <VoucherExpired vouchers={paginatedVouchers(getFilteredVouchers(true))} />
            <Box sx={{ mt: 3, display: "flex", justifyContent: "center" }}>
              <Pagination
                count={Math.ceil(getFilteredVouchers(true).length / itemsPerPage)}
                page={page}
                onChange={(e, value) => setPage(value)}
                color="primary"
              />
