...
export default function Delivery() {
  ...
  const handleChooseShipper = (shipper) => {
    ...
    // Open dialog if both shipper and order are selected
  ...
  const handleAutoAssign = () => {
    while (waitingShippers.length > 0 && orderManagement.length > 0) {
      assignDelivery(waitingShippers[0], orderManagement[0]);
    }
  };
            ...
            <List sx={{ p: 0 }}>
              {waitingShippers.map((shipper) => (
                ...
                >
                  ...
                  <Button
                    onClick={() => handleChooseShipper(shipper)}
                    sx={{
                      bgcolor: selectedPair.shipper?.shipper_id === shipper.shipper_id 
                        ? "#2E9567"  // Màu khi được chọn
                        : "#4AEDC4", // Màu mặc định
                      color: selectedPair.shipper?.shipper_id === shipper.shipper_id 
                        ? "white"    // Màu chữ khi được chọn
                        : "black",   // Màu chữ mặc định
                      "&:hover": { 
                        bgcolor: selectedPair.shipper?.shipper_id === shipper.shipper_id 
                          ? "#236F4D"  // Màu hover khi được chọn
                          : "#3BC5A0"  // Màu hover mặc định
                      },
                      textTransform: "none",
                      borderRadius: 2,
                      px: 3,
            ...
            <List sx={{ p: 0 }}>
              {orderManagement.map((order) => (
                ...
                >
                  ...
                  <Button
                    onClick={() => handleChooseOrder(order)}
                    sx={{
                      bgcolor: selectedPair.order?.order_id === order.order_id 
                        ? "#D97706"  // Màu khi được chọn
                        : "#FEF3C7", // Màu mặc định
                      color: selectedPair.order?.order_id === order.order_id 
                        ? "white"    // Màu chữ khi được chọn
                        : "black",   // Màu chữ mặc định
                      "&:hover": { 
                        bgcolor: selectedPair.order?.order_id === order.order_id 
                          ? "#B45309"  // Màu hover khi được chọn
                          : "#FDE68A"  // Màu hover mặc định
                      },
                      textTransform: "none",
                      borderRadius: 2,
                      px: 3,
