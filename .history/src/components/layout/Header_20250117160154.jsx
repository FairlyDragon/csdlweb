...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const fetchDeliveringCount = async () => {
      try {
        const response = await fetch(
        ...
        const data = await response.json();
        setDeliveringCount(data.number_of_delivering_orders || 0);
        console.log("Delivering count:", data.number_of_delivering_orders);
      } catch (error) {
        console.error("Error fetching delivering count:", error);
        setDeliveringCount(0);
      }
    };

    const fetchExpiredVoucherCount = async () => {
      try {
        const response = await fetch(
        ...
        const data = await response.json();
        setExpiredVoucherCount(data.length || 0);
        console.log("Expired voucher count:", data.length);
      } catch (error) {
        console.error("Error fetching expired voucher count:", error);
        setExpiredVoucherCount(0);
      }
    };

    const fetchWaitingOrderCount = async () => {
      try {
        const response = await fetch(
        ...
        const data = await response.json();
        setWaitingOrderCount(data.number_of_waiting_orders || 0);
        console.log("Waiting order count:", data.number_of_waiting_orders);
      } catch (error) {
        console.error("Error fetching waiting order count:", error);
        setWaitingOrderCount(0);
      }
    };
    ...
    const fetchAllCounts = async () => {
      fetchFoodCount();
      fetchVoucherCount();
      fetchDeliveringCount();
      fetchExpiredVoucherCount();
      fetchWaitingOrderCount();
    };

    // Fetch lần đầu
    fetchAllCounts();

    // Tạo interval để fetch mỗi 5 giây
