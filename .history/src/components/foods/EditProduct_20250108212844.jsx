...
const EditProduct = ({ open, product, onClose, onSave, onDelete }) => {
  ...
  const handleSave = () => {
    if (!formData.menuitem_id) {
      console.error("No menuitem_id found in formData:", formData);
      return;
    }
    console.log("Saving product with ID:", formData.menuitem_id);
    onSave(formData);
    onClose();
  };

  const handleDelete = () => {
    console.log("Deleting with menuitem_id:", formData.menuitem_id);
