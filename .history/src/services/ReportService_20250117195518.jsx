...
const ReportService = {
  ...
  getCustomerReport: async (startDate, endDate) => {
    try {
      console.log("Fetching customer report with dates:", { startDate, endDate });
      const response = await axios.get(
        `${API_URL}/report/customer?start_date=${startDate}&end_date=${endDate}`,
        {
          headers: {
  ...
  getShipperReport: async (startDate, endDate) => {
    try {
      const response = await axios.get(
        `${API_URL}/report/shipper?start_date=${startDate}&end_date=${endDate}`,
        {
          headers: {
            accept: "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  getRestaurantReport: async (startDate, endDate) => {
    try {
      const response = await axios.get(
        `${API_URL}/report/restaurant?start_date=${startDate}&end_date=${endDate}`,
        {
          headers: {
            accept: "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
