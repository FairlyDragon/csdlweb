    ...
    <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3, alignItems: 'center' }}>
        <Box sx={{ display: 'flex', gap: 4 }}>
          <Typography>
            Total Order: <strong>{filteredData.reduce((sum, row) => sum + row.totalOrder, 0)}</strong>
          </Typography>
          <Typography>
            Total Purchase: <strong>${filteredData.reduce((sum, row) => sum + row.totalPurchase, 0)}</strong>
          </Typography>
        </Box>
