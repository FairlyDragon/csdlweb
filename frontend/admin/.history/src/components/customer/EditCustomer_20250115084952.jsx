...
export default function EditCustomer() {
  ...
  const fetchCustomerData = async () => {
    try {
      setLoading(true);
      // Fetch customer details và history
      const details = await CustomerService.getCustomerById(id);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 3, mb: 4, position: "relative" }}>
        ...
        <Button
          ...
          color="primary"
          onClick={() => {
            // Xử lý sự kiện edit ở đây
            console.log("Edit customer:", customerDetails);
          }}
          sx={{
            position: "absolute",
            top: 16,
      ...
      <Box>
        ...
        <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 3 }}>
          ...
          <Button
            variant="outlined"
            endIcon={<Box component="span" sx={{ color: "#4dabf7" }}>⬇️</Box>}
            sx={{
              color: "#4dabf7",
              borderColor: "#4dabf7",
              textTransform: "none",
