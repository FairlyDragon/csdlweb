...
export default function Food() {
  ...
  const handleEditProduct = async (editedProduct) => {
    try {
      setIsLoading(true);
      console.log("Received edited product:", editedProduct); // Debug log

      if (!editedProduct || !editedProduct.menuitem_id) {
        console.error("Invalid product data:", editedProduct);
        throw new Error("Missing menuitem_id");
      }
  ...
  const handleOpenEditDialog = (product) => {
    ...
    setEditDialog({
      open: true,
      product: { ...product }, // Tạo bản sao của product
    });
  };

  // Handle delete product
  const handleDeleteProduct = async (menuitem_id) => {
    try {
      setIsLoading(true);
      ...
      await foodService.deleteFood(menuitem_id);
      setProducts((prevProducts) =>
        prevProducts.filter((product) => product.menuitem_id !== menuitem_id)
      );
      setEditDialog({ open: false, product: null });
    } catch (error) {
