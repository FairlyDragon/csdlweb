...
const INITIAL_DISHES = [
  {
    menuitem_id: 1,
    name: "Spicy Noodles",
    description: "Delicious spicy noodles with vegetables and special sauce",
    price: "5.59",
    category: "Noodles",
    average_rating: 4.5,
    discount: 0,
    image_url: "/images/spicy-noodles.jpg",
    is_active: true,
  },
  {
    menuitem_id: 2,
...
export default function Foods() {
  ...
  const fetchFoods = async () => {
    try {
      ...
      const data = await foodService.getFoods(currentPage, filters);
  ...
  useEffect(() => {
    fetchFoods();
  }, [currentPage, selectedCategory, stockFilter, sortPrice]);

  // Thêm món ăn mới
  const handleAddProduct = async (newProduct) => {
    try {
      setIsLoading(true);
      const addedProduct = await foodService.createFood(newProduct);
  ...
  useEffect(() => {
    if (error) {
      // Hiển thị thông báo lỗi (có thể dùng Snackbar của MUI)
      console.error(error);
    }
  }, [error]);
  ...
  useEffect(() => {
    ...
    if (savedProducts) {
      ...
      // Migrate data từ format cũ sang format mới
      const migratedProducts = oldProducts.map((product) => ({
        menuitem_id: product.id || product.menuitem_id,
        name: product.name,
        description: product.details || product.description,
        price: product.price,
        category: product.category,
        average_rating: product.rating || product.average_rating,
        discount: product.discount,
        image_url: product.image || product.image_url,
        is_active: product.isActive || product.is_active,
      }));
      setProducts(migratedProducts);
      localStorage.setItem("dishes", JSON.stringify(migratedProducts));
    } else {
      setProducts(INITIAL_DISHES);
      localStorage.setItem("dishes", JSON.stringify(INITIAL_DISHES));
    }
  }, []);
