...
export default function Delivery() {
  ...
  }, [activeShippers, deliveringOrders, waitingOrders, waitingShippers, orderManagement]);

  // Handle choosing a shipper
  const handleChooseShipper = (shipperId) => {
    setWaitingShippers(prev => prev.filter(shipper => shipper.shipper_id !== shipperId));
    setActiveShippers(prev => prev + 1);
  };

  // Handle choosing an order
  const handleChooseOrder = (orderId) => {
    setOrderManagement(prev => prev.filter(order => order.order_id !== orderId));
  ...
  const handleAutoAssign = () => {
    if (waitingShippers.length > 0 && orderManagement.length > 0) {
      const shipper = waitingShippers[0];
      const order = orderManagement[0];
      
      handleChooseShipper(shipper.shipper_id);
      handleChooseOrder(order.order_id);
    }
  };
                ...
                <ListItem
                  ...
                  secondaryAction={
                    <Button
                      variant="contained"
                      size="small"
                      onClick={() => handleChooseShipper(shipper.shipper_id)}
                      sx={{
                        bgcolor: "#4AEDC4",
                        color: "black",
                ...
                <ListItem
                  ...
                  secondaryAction={
                    <Button
                      variant="contained"
                      size="small"
                      onClick={() => handleChooseOrder(order.order_id)}
                      sx={{
                        bgcolor: "#FEF3C7",
                        color: "black",
