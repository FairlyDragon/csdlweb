...
const Voucher = () => {
  ...
  useEffect(() => {
    // Check for expired vouchers and move them
    const now = new Date();
    const updatedVouchers = vouchers.map(voucher => {
      const endDate = new Date(voucher.end_date);
      if (endDate <= now) {
        return { ...voucher, isExpired: true };
      }
      return voucher;
    });

    // Sort expired vouchers to the beginning of expired list
    const sortedVouchers = updatedVouchers.sort((a, b) => {
