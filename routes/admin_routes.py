...
@role_required(any_admin_roles)
async def dashboard_center_total_orders( periodicity: str = "daily", current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def dashboard_center_total_revenue( periodicity: str = "daily", current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def dashboard_center_customers_map( periodicity: str = "daily", current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def dashboard_footer( skip: int = 0, limit: int = 5, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def create_menuitem_route(menuitem: CreateMenuItemSchema, current_user: User = Depends(get_current_user)):
    return await create_menuitem(menuitem)


@router.put("/foods/menuitems", response_description="Update a menu item")
@role_required(any_admin_roles)
async def update_menuitem_route(menuitem: UpdateMenuItemSchema, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def delete_menuitem_route(menuitem_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def set_discount_percentage_route(discount_percentage: int, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def create_voucher_route(voucher: CreateVoucherSchema, current_user: User = Depends(get_current_user)):
    return await create_voucher(voucher)


@router.get("/vouchers/{status}", response_description="Get vouchers by status")
@role_required(any_admin_roles)
async def read_vouchers_by_status_route(status: str = Path(..., example="available"), current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_shippers_route(current_user: User = Depends(get_current_user)):
    return await read_shippers()


@router.get("/shippers/history/{shipper_id}", response_description="Get delivery history by shipper id")
@role_required(any_admin_roles)
async def read_delivery_history_by_shipper_id_route(shipper_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_shipper_infor_by_shipper_id_route(shipper_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def update_shipper_infor_by_shipper_id_route(shipper: ShipperSchema, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def delete_shipper_by_shipper_id_route(shipper_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_customers_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_order_history_by_customer_id_route(customer_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_customer_infor_by_customer_id_route(customer_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def update_customer_infor_by_customer_id_route(customer: CustomerResponseSchema, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def delete_customer_by_customer_id_route(customer_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_active_shippers_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_currently_waiting_shippers_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_delivering_orders_route(current_user: User = Depends(get_current_user)):
    return await read_delivering_orders()


@router.get("/deliveries/waiting_orders", response_description="Get waiting orders")
@role_required(any_admin_roles)
async def read_waiting_orders_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles) 
async def read_num_of_waiting_orders_route(current_user: User = Depends(get_current_user)):
    return await read_num_of_waiting_orders()


@router.post("/deliveries/assign", response_description="Assign an order to a shipper")
@role_required(any_admin_roles)
async def assign_delivery_route(order_id: str, shipper_id: str, current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_pending_orders_preview_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_pending_orders_detais_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_passed_pending_orders_preview_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def read_passed_pending_orders_details_route(current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def update_order_route(order_id: str, status: str = Path(..., example="processing"), current_user: User = Depends(get_current_user)):
...
async def update_role_of_user_route(id: str, role: LimitedRole = Query(..., example="admin"), current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def get_customer_report_route(time_period: TimePeriod = Depends(get_time_period), current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles)
async def get_shipper_report_route(time_period: TimePeriod = Depends(get_time_period), current_user: User = Depends(get_current_user)):
...
@role_required(any_admin_roles) 
async def get_restaurant_report_route(time_period: TimePeriod = Depends(get_time_period), current_user: User = Depends(get_current_user)):
